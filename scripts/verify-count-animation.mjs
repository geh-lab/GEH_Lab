...
function fixture({ reduced = false, observer = true, target = 102, lang = 'kr', page = 'home' } = {}) {
  const payload = { projectId: 'count-fixture', collections: { publications: { items: [], fetchedAt: 1234 } } };
  const { document } = parseHTML(`<html><body data-page="${page}" data-lang="${lang}"><main><strong class="count-up" data-count-key="total" data-target="${target}">${target}</strong></main><script id="public-page-data" type="application/json">${JSON.stringify(payload)}</script></body></html>`);
  ...
  const api = vm.runInNewContext(`(() => { ${source}\nreturn { setupCountAnimations }; })()`, {
    ...utils, ...patents, ...inventors, ...data, document, window, console, URL, URLSearchParams,
...
console.log(`Count animation: ${checks} regression checks passed (SSR, viewport, repeated render, accessibility, cleanup).`);
