...
export function normalizeMember(item = {}, options = {}) {
  ...
  return {
    ...
    bachelorsMajorEn: item.bachelorsMajorEn || item.bachelorsMajor || '',
    mastersSchoolKr: item.mastersSchoolKr || '',
    mastersSchoolEn: item.mastersSchoolEn || item.mastersSchool || '',
    mastersMajorKr: item.mastersMajorKr || '',
    mastersMajorEn: item.mastersMajorEn || item.mastersMajor || '',
    doctoralSchoolKr: item.doctoralSchoolKr || '',
    doctoralSchoolEn: item.doctoralSchoolEn || item.doctoralSchool || '',
    doctoralMajorKr: item.doctoralMajorKr || '',
    doctoralMajorEn: item.doctoralMajorEn || item.doctoralMajor || '',
    bachelorsSchool: item.bachelorsSchool || item.bachelorsSchoolEn || item.bachelorsSchoolKr || '',
    bachelorsMajor: item.bachelorsMajor || item.bachelorsMajorEn || item.bachelorsMajorKr || '',
    mastersSchool: item.mastersSchool || item.mastersSchoolEn || item.mastersSchoolKr || '',
    mastersMajor: item.mastersMajor || item.mastersMajorEn || item.mastersMajorKr || '',
    doctoralSchool: item.doctoralSchool || item.doctoralSchoolEn || item.doctoralSchoolKr || '',
...
function normalizeBoardCategory(value = '') {
  ...
  if (['notice', '공지'].includes(raw)) return 'other';
  if (['equipment', 'equipmentintro', 'equipment-intro', 'lab-equipment', 'labequipment', 'news', '소식', '실험실 장비 목록', '실험실장비목록'].includes(raw)) return 'equipment';
  if (['conference', 'poster', 'oral', '학회', '포스터', '구두'].includes(raw)) return 'conference';
  if (['workshop', 'seminar', '워크숍', '워크샵', '세미나'].includes(raw)) return 'workshop';
  if (['other', 'misc', '기타'].includes(raw)) return 'other';
  return raw;
}
...
export function normalizeBoardPost(item = {}, options = {}) {
  ...
  const imagePaths = (Array.isArray(item.imagePaths) ? item.imagePaths : [item.imagePath || ''])
    .filter(Boolean)
    .map((value) => String(value));
  return {
    ...
    imageUrls,
    imagePath: imagePaths[0] || item.imagePath || '',
    imagePaths,
    date: item.date || '',
    deleted: item.deleted === true,
    deletedAt: item.deletedAt || '',
